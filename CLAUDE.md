...
| MCP server exits ~300-400ms after `initialize` handshake from Claude Desktop | `startMcpServer()` returns after `server.connect()`, causing `main()` to complete. Node.js event loop empties and process exits. Manual stdin tests mask the issue because pipes keep event loop active. | Add `await new Promise(() => {})` at end of `startMcpServer()` to create infinite promise that prevents function from returning | 2026-03-30 |
| `Cannot find package .../node_modules/fast-xml-parser/src/fxp.js` in MCPB bundle | `.mcpbignore` pattern `src/` (without leading `/`) matches ALL `src/` directories recursively, including inside `node_modules`. This strips source files from dependencies that store compiled JS in `src/` directories. | Change `.mcpbignore` pattern from `src/` to `/src/` (with leading `/`) to match ONLY root project's src directory. Pattern convention: leading `/` = root only, no leading `/` = matches anywhere. | 2026-03-31 |
