...
export async function handler(
  ...
  try {
    ...
    if (event.path === '/fetch' || event.path === '/dev/fetch' || event.path === '/prod/fetch') {
      ...
      const contextResult = await ledger.checkContextualIntegrity(sessionId, 'visus_fetch', fetchReq, result.value);
      const { score } = contextResult;
      if (score > 0.7) {
        return {
          statusCode: 403,
          headers: corsHeaders,
      ...
      // Update ledger (stub)
      ledger.update(sessionId, [], 'visus_fetch', []);

      // Fire-and-forget audit logging (6 args)
      logAuditEvent(
        userId,
        requestId,
        fetchReq.url,
    ...
    if (event.path === '/fetch-structured' || event.path === '/dev/fetch-structured' || event.path === '/prod/fetch-structured') {
      ...
      // VSIL Check
      const { score } = await ledger.checkContextualIntegrity(sessionId, 'visus_fetch_structured', fetchReq, result.value);
      if (score > 0.7) {
        return {
          statusCode: 403,
          headers: corsHeaders,
          body: JSON.stringify({ blocked: true, session_risk: score, reason: 'High session risk' }),
        };
      }

      // Update ledger
      ledger.update(sessionId, [], 'visus_fetch_structured', []);
    ...
    // Update ledger
    const hashes = []; // Extract entity hashes from args/result
